...
def main():
    ...
    parser.add_argument("input_dir", help="Input annotated directory, include image and json files")
    parser.add_argument("output_dir", help="Output dataset directory")
    parser.add_argument(
        "--labels", help="Labels file or comma separated text", required=True
    )
    parser.add_argument(
        "--valid_ratio", type=float, help="valid set ratio", required=True
    )
    parser.add_argument(
    ...
    os.makedirs(os.path.join(args.output_dir, "ImageSets", "Segmentation"))
    names = [name.strip(".json") for name in os.listdir(args.input_dir) if name.endswith("json")]
    valid_ratio = args.valid_ratio
    assert 0 < valid_ratio < 1, "valid ratio should between 0 and 1"
    total_count = len(names)
    valid_count = int(total_count * valid_ratio)
    valid_names = rd.choice(names, valid_count, replace=False)
    train_names = list(set(names) - set(valid_names))
    with open(os.path.join(args.output_dir, "ImageSets", "Segmentation", "train.txt"), "w", encoding="utf-8") as file:
        for i, name in enumerate(train_names):
            if i != len(train_names) - 1:
                file.write(name + "\n")
            else:
                file.write(name)
    with open(os.path.join(args.output_dir, "ImageSets", "Segmentation", "val.txt"), "w", encoding="utf-8") as file:
        for i, name in enumerate(valid_names):
            if i != len(valid_names) - 1:
                file.write(name + "\n")
            else:
                file.write(name)
    with open(os.path.join(args.output_dir, "ImageSets", "Segmentation", "trainval.txt"), "w", encoding="utf-8") as file:
    ...
    for filename in sorted(glob.glob(osp.join(args.input_dir, "*.json"))):
        ...
        if not args.nonpy:
            ...
            cv2.imwrite(out_cls_file, cls.astype(np.uint8))
        if not args.noviz:
        ...
        if not args.noobject:
            ...
            if not args.nonpy:
                ...
                cv2.imwrite(out_ins_file, ins.astype(np.uint8))
            if not args.noviz:
